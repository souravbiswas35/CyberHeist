...
public class BruteForceToolController implements Initializable {
    ...
    public void setUsername(String username) {
        ...
        updateUI();
        System.out.println("BruteForce-"+username); //it loads later
    }

    @Override
    public void initialize(URL location, ResourceBundle resources) {
        // Set image and description for Brute Force tool
        toolImage.setImage(new Image("file:D:/JavaFx Projects/CyberHeist/img/BruteForce.jpg"));
        toolDescription.setText(
                "Brute Force: Crash through security by trying every possible combination—no finesse, just raw power.\n" +
                        "Slow and noisy, but guaranteed to find a way in if given enough time"
        );
    }
    ...
    private void loadToolData() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                showAlert(Alert.AlertType.ERROR, "Tool Not Found", "Brute Force tool not found in database.");
                toolId = -1;
    ...
    private void handleBuyTool() {
        ...
        try {
            ...
            showAlert(Alert.AlertType.INFORMATION, "Purchase Successful",
                    "You have purchased Brute Force for " + toolCost + " coins!");
            updateUI();
